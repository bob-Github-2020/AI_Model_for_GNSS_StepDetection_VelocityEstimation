#!/usr/bin/python3
"""
GNSS ChangePoint (Step) Detection and Velocity Estimation with CNN

Author: Guoquan Wang, et al. (gwang@uh.edu)
Date: 12-1-2025

You may find the detailed methods in the original publication:
...
1. Version Compatibility:
   ...
   - Retrain if needed (see Train_ChangePointCNN-GNSS_VGG.py)
...
1. Prepare working directory with:
   - GNSS *.col files
   - ChangePointCNN_VGG_V?.keras 
2. Execute: `python3 GNSS_CPD_VelocityEstimation_VGG.py`
...
def remove_outliers_sigma_detrend(data, column, n_sigma=2.5, window=350):
    ...
    """
    max_iterations = 8  # Number of detrending iterations to ensure robust outlier removal.
...
def plot_time_series_with_longest_segment(data, velocities, intercepts, longest_segments, file_path):
    ...
    for i, direction in enumerate(directions):
        ...
        ax.plot(data_mean_removed.loc[valid, "Decimal_Year"], data_mean_removed.loc[valid, direction], 'bo', markersize=1,
        ...
        if longest_segment is not None:
            ...
            if len(segment_data) > 1:
                ...
                ax.plot(segment_data["Decimal_Year"], trend_line, "r--", linewidth=3,
                        label=f"Velocity: {slope*10:.2f} mm/yr")
        ax.set_ylabel(f"{direction_labels[direction]} (cm)")
        ax.legend()
    axes[-1].set_xlabel("Decimal Year")
    parts = os.path.basename(file_path).split('_')[:2]
    station_RF = f"{parts[0]} ({parts[1]})"
    station_name = f"{parts[0]}"
    plt.suptitle(f"{station_RF}", fontsize=10, y=0.935)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(f"{station_name}.png", dpi=300, bbox_inches='tight', pad_inches=0.1, facecolor='white')
    plt.close()
    del fig, axes
    gc.collect()
...
def plot_time_series_with_velocity(data, change_points, velocities, intercepts, longest_segments, file_path):
        ...
        for j, cp_idx in enumerate(change_points.get(direction, [])):
            if cp_idx in data.index:
                ax.axvline(data.loc[cp_idx, "Decimal_Year"], color='r', linestyle='--',
                           label="Detected change point" if j == 0 else "")
    ...
    plt.suptitle(f"Detrended {station_name}-NEU: Change-Point Detection", fontsize=10, y=0.935)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(f"{station_name}_detrended.png", dpi=300, bbox_inches='tight', pad_inches=0.1, facecolor='white')
    plt.close()
    del fig, axes
    gc.collect()
...
def write_ai_parameters(
    ...
    Args:
        ...
        small_window_size (int): Window size for small step detection---moving window.
...
# ============================================================================
# 17. Use CNN to Predict Plot Suitability for Velocity Estimation
# ============================================================================
...
def select_optimal_small_params_sequential(step_corrected_data, cnn_model, station_name, threshold_good=0.85):
    ...
    """
    # control parameters for instant change points
    candidate_N = [0.3, 0.4, 0.5, 0.25, 0.2, 0.15, 0.7, 1.0, 1.4, 2.0, 5, 30]
    candidate_E = [0.3, 0.4, 0.25, 0.2, 0.15, 0.5, 0.7, 1.0, 1.4, 2.0, 5, 30]
    candidate_U = [1.0, 0.8, 0.7, 0.6, 0.5, 0.3, 1.2, 1.4, 1.8, 2.5, 5, 10, 30]

    small_window_size = 30      # moving-window size for detcting instant CPs
    small_min_distance = 30     # min-distance for 2  adjacent CPs
    
    # control parameters for transitional change points
    # magnitude of curve, So far, only the first one is used!
    candidate_C_N = [0.4, 0.5, 1.0]
    candidate_C_E = [0.4, 0.5, 1.0]
    candidate_C_U = [0.5, 0.6, 1.2]
    improvement_ratio = 0.2
...
# ============================================================================
def process_file(file_path, cnn_model):
    """
    ...
    optimal_small, cnn_scores, change_points = select_optimal_small_params_sequential(
        cleaned_data, cnn_model, station_name, threshold_good=0.85
    )
...
if __name__ == "__main__":
    ...
    #cnn_model = tf.keras.models.load_model("/home/bob/StepCNN_Training/ChangePointCNN-GNSS_VGG_V7.keras")
    ...
    file_paths = glob.glob("*_NEU_cm.col")
    if not file_paths:
    ...
    for i, file_path in enumerate(file_paths, 1):
        ...
        process_file(file_path, cnn_model)
